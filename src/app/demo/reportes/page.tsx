...
import { exportarReporteExcel } from "@/lib/exportExcel";
...
export default function DemoReportesPage() {
  ...
  useEffect(() => {
    seedIfEmpty();
    setFeriasList(getResumenFerias().filter((f) => f.estado === "finalizada"));
    setProdList(getProductos().filter((p) => p.activo));
    setLoading(false);
  }, []);
  ...
  const handleExport = async () => {
    setExporting(true);
    try {
      await exportarReporteExcel(feriasList, prodList);
    } finally {
      setExporting(false);
    }
  };
  ...
  if (loading) return (
    <div className="animate-pulse space-y-4 pt-4">
      ...
      <div className="grid grid-cols-2 gap-3">{[...Array(4)].map((_, i) => <div key={i} className="h-24 bg-vs-border rounded-2xl" />)}</div>
    </div>
  );
