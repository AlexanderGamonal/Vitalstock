...
import { seedIfEmpty, getProductos, updateProducto } from "@/lib/demo/store";
...
export default function DemoProductosPage() {
  ...
  useEffect(() => {
    seedIfEmpty();
    setLista(getProductos().sort((a, b) => a.nombre.localeCompare(b.nombre)));
  }, []);

  const toggleActivo = useCallback((p: Producto, e: React.MouseEvent) => {
    e.preventDefault(); e.stopPropagation();
    if (p.activo && !confirm(`¿Desactivar "${p.nombre}"? Ya no aparecerá en la canasta.`)) return;
    updateProducto(p.id, { activo: !p.activo });
    setLista(getProductos().sort((a, b) => a.nombre.localeCompare(b.nombre)));
  }, []);
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-5">
        ...
        <Link href="/demo/productos/nuevo">
          <button className="bg-vs-green text-white font-body font-bold text-sm px-4 py-2 rounded-xl">+ Agregar</button>
        </Link>
      </div>
      ...
      <div className="grid grid-cols-3 gap-2 mb-5">
        ...
        <div className="bg-white border border-vs-border rounded-xl p-3 text-center">
          <div className="font-display font-black text-vs-accent text-xl">{activos.reduce((a, p) => a + p.stock_actual, 0)}</div>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0">
                        <div className="font-body font-bold text-vs-text text-sm leading-tight flex items-center gap-1">
                          {p.nombre}{p.destacado && <span className="text-xs">⭐</span>}
                        </div>
                        {p.descripcion && <div className="font-body text-vs-muted text-xs mt-0.5 truncate">{p.descripcion}</div>}
                        {p.categoria && <span className="inline-block mt-1 bg-vs-greenPale text-vs-green font-body font-bold text-[10px] px-2 py-0.5 rounded-full">{p.categoria}</span>}
                      </div>
                      <span className={`font-body font-bold text-xs px-2 py-1 rounded-full flex-shrink-0 ${isOut ? "bg-red-100 text-red-700" : isLow ? "bg-red-50 text-red-600" : "bg-vs-greenPale text-vs-green"}`}>
                    ...
                    <div className="flex gap-4 mt-2">
                      ...
                      <div>
                        ...
                        <div className="font-body font-bold text-vs-accent text-sm">{calcMargen(p.precio_descuento ?? p.precio_venta, p.precio_costo)}%</div>
                      </div>
                    </div>
                    {p.precio_descuento != null && <span className="inline-block mt-1 bg-orange-100 text-orange-600 font-body font-bold text-[10px] px-2 py-0.5 rounded-full">🏷️ En oferta</span>}
                    {isLow && !isOut && <p className="font-body text-red-600 text-xs font-semibold mt-2">⚠ Stock bajo — mínimo: {p.stock_minimo} u</p>}
                    <button onClick={(e) => toggleActivo(p, e)} className="mt-2 font-body text-vs-muted text-[10px] underline underline-offset-2 hover:text-red-500 transition-colors">Desactivar</button>
                  </div>
                </div>
              </div>
            </Link>
      ...
      {inactivos.length > 0 && (
        <div className="mt-6">
          <h2 className="font-display font-bold text-vs-muted text-base mb-3">Desactivados ({inactivos.length})</h2>
          <div className="space-y-2">
            {inactivos.map((p) => (
              <div key={p.id} className="bg-white border border-vs-border rounded-2xl p-4 opacity-60 flex items-center gap-3">
                ...
                <button onClick={(e) => toggleActivo(p, e)} className="flex-shrink-0 bg-vs-greenPale text-vs-green font-body font-bold text-xs px-3 py-1.5 rounded-xl hover:bg-vs-green hover:text-white transition-colors">Reactivar</button>
              </div>
            ))}
          </div>
        </div>
      ...
      {lista.length === 0 && (
        <div className="text-center py-16">
          ...
          <Link href="/demo/productos/nuevo"><button className="mt-4 bg-vs-green text-white font-body font-bold px-6 py-3 rounded-xl">+ Agregar producto</button></Link>
        </div>
      )}
    </div>
  );
