...
import { seedIfEmpty, getResumenFerias } from "@/lib/demo/store";

export default function DemoFeriasPage() {
  const [lista, setLista] = useState<ResumenFeria[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    seedIfEmpty();
    setLista(getResumenFerias());
    setLoading(false);
  }, []);
  ...
  if (loading) return <div className="animate-pulse space-y-3 pt-4">{[...Array(3)].map((_, i) => <div key={i} className="h-24 bg-vs-border rounded-2xl" />)}</div>;

  return (
    <div>
      <div className="flex items-center justify-between mb-5">
        <h1 className="font-display font-black text-vs-text text-2xl">Ferias</h1>
        <Link href="/demo/ferias/nueva">
          <button className="bg-vs-green text-white font-body font-bold text-sm px-4 py-2 rounded-xl hover:bg-opacity-90 transition-all">+ Nueva feria</button>
        </Link>
      </div>
                ...
                <div className="bg-vs-green text-white rounded-2xl p-4 relative overflow-hidden hover:bg-opacity-90 transition-all">
                  ...
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <div className="font-body text-sm opacity-85 mt-1">📍 {f.ubicacion} · 📅 {new Date(f.fecha + "T00:00:00").toLocaleDateString("es-PE", { day: "numeric", month: "long" })}</div>
                    </div>
                  </div>
                  <div className="mt-3 bg-white bg-opacity-20 rounded-xl px-4 py-2.5 text-center">
                    <span className="font-body font-bold text-sm">{f.total_llevado > 0 ? "📝 Anotar lo vendido →" : "🎯 Armar canasta para esta feria →"}</span>
                  </div>
                </div>
              </Link>
            ))}
                  ...
                  <div className="bg-white border border-vs-border rounded-2xl p-4 hover:border-vs-green transition-colors">
                    <div className="flex justify-between items-start">
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="font-body text-vs-muted text-xs mt-0.5">Vendidos: {f.total_vendido} uds de {f.total_llevado}</div>
                      </div>
                      <div className="text-right ml-3 flex-shrink-0">
                        <div className="font-display font-black text-vs-green text-lg">{fmt(Number(f.total_ingresos))}</div>
                        <div className={`font-body font-semibold text-xs ${neta >= 0 ? "text-green-500" : "text-red-500"}`}>Neto: {fmt(neta)}</div>
                      </div>
                    </div>
                  </div>
                </Link>
      ...
      {lista.length === 0 && (
        <div className="text-center py-16">
          ...
          <Link href="/demo/ferias/nueva"><button className="bg-vs-green text-white font-body font-bold px-6 py-3 rounded-xl">+ Nueva feria</button></Link>
        </div>
      )}
    </div>
  );
